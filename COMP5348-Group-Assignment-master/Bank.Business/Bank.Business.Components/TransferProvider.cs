        ...
        public void Transfer(double pAmount, int pFromAcctNumber, int pToAcctNumber, string Oid)
        {
            ...
            using (BankEntityModelContainer lContainer = new BankEntityModelContainer())
            {
                try
                {
                    ...
                    Console.WriteLine("Successfully Transferred $" + pAmount);

                    FundsTransferService.FundsTransferServiceClient lClient = new FundsTransferService.FundsTransferServiceClient();
                    lClient.TransferOutcome(true, Oid);
                ...
                catch (Exception lException)
                {
                    ...
                    FundsTransferService.FundsTransferServiceClient lClient = new FundsTransferService.FundsTransferServiceClient();
                    lClient.TransferOutcome(false, Oid);
                    throw;
                }
            }
        }
        ...
        public void Refund(double pAmount, int pFromAcctNumber, int pToAcctNumber, string Oid)
        {
            ...
            using (BankEntityModelContainer lContainer = new BankEntityModelContainer())
            {
                try
                {
                    ...
                    FundsTransferService.FundsTransferServiceClient lClient = new FundsTransferService.FundsTransferServiceClient();
                    lClient.RefundOutcome(true, Oid);
                    // save changed entities and finish the transaction
                    lContainer.SaveChanges();
                    lScope.Complete();
                }
                catch (Exception lException)
                {
                    Console.WriteLine("Error occured while transferring money:  " + lException.Message);
                    FundsTransferService.FundsTransferServiceClient lClient = new FundsTransferService.FundsTransferServiceClient();
                    lClient.RefundOutcome(false, Oid);
                    throw;
                }
            }
        }
